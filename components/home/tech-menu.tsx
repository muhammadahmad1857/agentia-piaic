...
// ─── OFFSCREEN ICON RENDERING HELPER ──────────────────────────

/**
 * Renders the given React icon into an offscreen container,
 * extracts its SVG outerHTML, and returns a data URL.
 */
function getSVGDataUrlFromIcon(icon: React.ElementType, props: any = { size: 256, color: "#000" }): Promise<string> {
...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    // Texture – using offscreen icon rendering
    this.initTexture();
  ...
  private initTexture(): void {
    ...
    // Use offscreen rendering to convert each icon to a data URL
    Promise.all(
      this.items.map((item) =>
        getSVGDataUrlFromIcon(item.icon, { size: 256, color: "#000" }).then((dataUrl) => {
      ...
      gl.generateMipmap(gl.TEXTURE_2D);
      // Uncomment the next line to debug the atlas by appending it to the DOM.
...
// ─── DEFAULT ITEMS (using react-icons) ─────────────────────────
        ...
        <>
          ...
          <h2
            className={`
              ...
              left-4 top-1/2
              transform translate-x-5 -translate-y-1/2
          ...
          <div
            ...
            `}
          >
            <p className="select-none relative text-[#060606] text-2xl">
              &#x2197;
            </p>
          </div>
        </>
